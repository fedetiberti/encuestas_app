...
datos_sin_proyectar_indecisos <- encuestas %>%
  ...
  mutate(party = case_when(party=="cf" ~ "Hacemos",
...
datos_proyeccion_indecisos <- encuestas2 %>%
  pivot_longer(cols =4:9,
               names_to = "party",
               values_to = "percentage_points")%>%
  mutate(party = case_when(party=="cfn" ~ "Hacemos",
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("partyInput", "Seleccione partidos",
                         choices = c("Frente de Todos", "Juntos por el Cambio", "La Libertad Avanza", "Frente de Izquierda", "Hacemos", "Otros - Blanco - Indecisos"),
                         selected = c("Frente de Todos", "Juntos por el Cambio", "La Libertad Avanza", "Frente de Izquierda", "Hacemos", "Otros - Blanco - Indecisos")),
      ...
      actionButton("selectMore3", "Incluir enc con 5 o + encuestas"),
      ...
      checkboxGroupInput("pollsterInput", "Seleccione encuestadoras",
                         ...
                         selected = encuestas_long %>% pull(encuestadora) %>% unique()),
  ...
  output$pollPlot <- renderggiraph({
    ...
    if (nrow(filtered_data) == 0) {
      ...
      p <- ggplot(filtered_data(), aes(x = fecha, y = percentage_points, color = party, group = party, tooltip = paste("Partido:", party, "<br>",
        ...
        geom_smooth(data=subset(filtered_data(), (fecha>as.Date("2023-08-13") & input$separate_trends==TRUE & encuestadora!="Elecciones legislativas" & encuestadora != "Elecciones generales" & encuestadora!= "Elecciones primarias")),method = "loess", se = input$showSE, aes(fill = party), show.legend = FALSE, span=input$slider2,family = "symmetric") +
